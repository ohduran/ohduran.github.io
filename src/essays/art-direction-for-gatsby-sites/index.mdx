...
One of the parameters on gatsby-node.js' Gatsby Node [createPages](https://www.gatsbyjs.com/docs/reference/config-files/gatsby-node/#createPages) API
...
So I did the following: I created a frontmatter variable called `template` that would be used by the createPages if it's
there, like so:
