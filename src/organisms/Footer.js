...
const Footer = ({ className }) => {
  return (
    <footer className={`${className ? className : ""}`}>
      <div className="sm:w-11/12 pt-5 mx-auto ">
        <main
        ...
        >
          ...
          <span className="font-family-secondary col-start-4 text-nord-8">
            E
          </span>
        ...
        <aside className="text-nord-6 text-center md:text-left text-sm md:text-base grid sm:grid-cols-5 gap-2 mt-10 pb-5 md:w-11/12 mx-auto">
          {FooterLinks.map((item) => (
            <div className="" key={item.to}>
              ...
              <p className="hidden sm:block mt-1 text-xs md:text-sm opacity-75 font-family-secondary">
                {item.description}
              </p>
            </div>
          ))}
